...
void initGL()
{
	...
	int flag = fs ? SDL_FULLSCREEN : 0;

	const SDL_VideoInfo * info = SDL_GetVideoInfo();

	SDL_Surface * back = SDL_SetVideoMode( info->current_w, info->current_h, 32, SDL_DOUBLEBUF | SDL_OPENGL | SDL_HWSURFACE | flag );
...
protected:
	...
	virtual void apply( )
	{
		...
		//btVector3 resPos = getPosition();
		//Vector4   pos    = Vector4( resPos.x(), resPos.y(), resPos.z(), 1 );

	//	glLightfv( GL_LIGHT0 + number, GL_POSITION, pos );
...
public:
	...
	friend void loadO3D( const char * filename, vector< Mesh* > & out );
	...
	void update( )
	{
		...
		btTransform result = body->getWorldTransform();
		result.setBasis( result.getBasis().scaled( btVector3( collisionShape->getLocalScaling())));

		result.getOpenGLMatrix( matrix );	
...
void loadO3D( const char * filename, vector< Mesh* > & out )
{
	FILE * file = fopen( filename, "rb" );
...
{	
	...
	for( uint i = 0; i < level.size(); i++ )
	{
		...
		level.at( i )->setScale( btVector3( .4, .4, .4 ) );
	}
