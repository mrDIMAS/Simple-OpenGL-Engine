...
public:
	...
	virtual btVector3 getPosition( bool global = false )
	{
	...
	{
		//transform.getBasis( ).setRotation( btQuaternion( -euler.y() * DEG2RAD, -euler.x() * DEG2RAD, -euler.z() * DEG2RAD ));
	...
	virtual void turn( const btVector3 & euler )
	{
		...
		//transform.getBasis() *= btMatrix3x3( btQuaternion( -euler.y() * DEG2RAD, -euler.x() * DEG2RAD, -euler.z() * DEG2RAD));
		//if( parent )
	...
	{
		//transform.setOrigin( transform.getOrigin() - transform.getBasis().getColumn( 0 ) * speed.x() + transform.getBasis().getColumn( 2 ) * speed.z() );
...
protected:
	virtual void render( )
	{
		...
		/*
		btVector3 eye  = newTransform.getOrigin();
		btVector3 look = eye + newTransform.getBasis().getColumn( 2 );
		btVector3 up   = newTransform.getBasis().getColumn( 1 );
...
void controlCamera()
{
	...
	camera->turn( btVector3( mouse.getYSpeed() * 0.2, mouse.getXSpeed() * 0.2, 0 ));
...
{	
	...
	//scene.getChild( 2 )->turn( btVector3( 0, 45, 0 ));
	//scene.getChild( 2 )->move( btVector3( 0, 0, 200 ));
	
	camera.attachTo( scene.getChild( 2 ), 0 );

	camera.setPosition( btVector3( 0, 40, 0 ));
	...
	while( true )
	{
		...
		//scene.getChild( 2 )->turn( btVector3( 0, .1, 0 ));
		...
		scene.getChild( 2 )->move( btVector3( strafe, 0, speed ));

		camera.turn( btVector3( mouse.getYSpeed() * 0.2, 0, 0 ));
		scene.getChild( 2 )->turn( btVector3( 0, mouse.getXSpeed() * 0.2, 0 ));
